...
window.onload = function () {
  // Симуляция получения данных о трафике в баллах
  function fetchTrafficData() {
      // Пример случайного числа от 1 до 10
      const trafficScore = Math.floor(Math.random() * 10) + 1;

      // Выводим баллы трафика в элемент
      const trafficElement = document.getElementById('traffic-score');
      trafficElement.innerText = `${trafficScore} `;
  }

  // Симуляция получения данных через 3 секунды
...
// });
 ...
 // Скрипт для отображения информации о выбранном файле
 ...
 fileInput.addEventListener('change', function () {
     ...
     if (file) {
         fileInfo.textContent = `Вы выбрали файл: ${file.name}, размер: ${Math.round(file.size / 1024)} KB`;
     } else {
